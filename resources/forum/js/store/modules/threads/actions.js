...
export default {
    [actions.UPDATE_POST]({commit}, {post, body}) {
        return new Promise((resolve, reject) => {
            window.axios.put(route('chatter.api.posts.update', post.id), {
                body
            }).then(response => {
    ...
    [actions.FETCH_REPLIES]({commit}, {post, page}) {
        return new Promise((resolve, reject) => {
            window.axios.get(route('chatter.api.posts.replies', post.id), {
                params: {
                    page
                }
            }).then(response => {
    ...
    [actions.CREATE_POST]({commit}, {parent, thread, body}) {
        return new Promise((resolve, reject) => {
            ...
            if (parent) {
                endpoint = route('chatter.api.posts.replies.store', parent.id)
            } else if (thread) {
                endpoint = route('chatter.api.threads.posts.store', thread.id)
            }
    ...
    [actions.DELETE_POST]({commit}, {post}) {
        return new Promise((resolve, reject) => {
            window.axios.delete(route('chatter.api.posts.destroy', post.id), {maxRedirects: 0})
                .then(response => {
